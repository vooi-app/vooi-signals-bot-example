"""
One-off: recompute SL for every open position and replace the live trigger order.

Used after switching to USE_SIGNAL_SL=false: existing positions still carry the
wider signal SL; this script tightens them to DEFAULT_SL_PCT of margin. Run
with the main bot stopped.

    .venv/bin/python -m scripts.reset_sl           # dry run
    .venv/bin/python -m scripts.reset_sl --apply   # actually cancel+replace
"""
import asyncio
import sys
...
async def _reset_one(position: Position, *, apply: bool) -> None:
    ...
    new_sl_raw = compute_sl_price_from_pct(
        entry_price=position.entry_price,
        side=side,
        leverage=position.leverage,
    )
    new_sl = round_price(new_sl_raw, price_decimals, exit_side)
    size = round_size(position.size, size_decimals)

    old_sl = position.sl_price_current or position.sl_price_initial
    print(
        f"  pos {position.id} {position.exchange} {position.symbol} {side} "
        f"entry={position.entry_price} lev={position.leverage}  "
        f"SL {old_sl} -> {new_sl}  size={size}"
    )

    if old_sl is not None and abs(new_sl - old_sl) / old_sl < Decimal("0.0001"):
    ...
    if position.sl_moved_to_be_at is not None:
        print("    skip: SL already moved to breakeven — leaving alone")
        return
    ...
    async with session_scope() as session:
        old_order = await session.get(Order, position.sl_order_id)
        if old_order is None or not old_order.vooi_order_id:
            print(f"    ERROR: no old SL order/vooi_order_id for position {position.id}")
            return
    ...
    try:
        ...
        print(f"    ERROR cancelling old SL: {e}")
        return

    # 2) Place new SL via verified path
    async with session_scope() as session:
        position_db = await session.get(Position, position.id)
        old_order = await session.get(Order, position_db.sl_order_id)
        if old_order is not None:
            old_order.status = "cancelled"
            old_order.cancelled_at = datetime.now(timezone.utc)

        new_client_oid = make_client_order_id(
            position_db.signal_id or 0, position_db.exchange, suffix="sl-reset"
        )
        new_order = Order(
        ...
        if placed:
            position_db.sl_order_id = new_order.id
            position_db.sl_price_initial = new_sl
            position_db.sl_price_current = new_sl
            position_db.sl_strategy = "computed_margin_pct"
            position_db.last_synced_at = datetime.now(timezone.utc)
            print(f"    OK: new SL placed @ {new_sl}, vooi_order_id={new_order.vooi_order_id}")
            await emit_event(
                "SL_RESET_TIGHTEN",
                ...
                exchange=position_db.exchange,
                symbol=position_db.symbol,
                message=f"old_sl={old_sl} new_sl={new_sl} sl_pct_margin={settings.default_sl_pct}",
            )
        else:
            print(f"    ERROR: new SL not verified on exchange — position NAKED on SL side!")
...
async def main(apply: bool) -> None:
    async with session_scope() as session:
        result = await session.execute(
            ...
            .where(Position.sl_order_id.is_not(None))
            .order_by(Position.id)
        )
        positions = list(result.scalars())

    if not positions:
        print("No open positions with SL orders. Nothing to do.")
        return

    print(
        f"Found {len(positions)} open position(s) {'(APPLY)' if apply else '(DRY-RUN)'}. "
        f"DEFAULT_SL_PCT={settings.default_sl_pct}% of margin."
    )
    for p in positions:
        try:
